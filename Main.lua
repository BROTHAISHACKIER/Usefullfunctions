...
function funcs:logger(thing, ...)
	...
	if self.LOGGGGGG and table.find(listfiles("./"), "./"..self.LOGGGGGG) then
		...
		if table.find(listfiles("./"), "./.ULogger") or table.find(listfiles("./"), ".ULogger") then
			local log = "Log_"..string.sub(math.random(1,99999)..".log", 3, -1)
...
function funcs:funcedit(fn, env)
	local global = {
	    ...
	    game = {
	        ...
	        GetService = function(_, ...) return game:GetService(...) end,
	        HttpGet = function(_, ...) return game:HttpGet(...) end,
	    ...
	    coroutine = {
	        ...
	        isyieldable = coroutine.isyieldable, -- Fixed typo
	        resume = coroutine.resume,
	        running = coroutine.running,
	        status = coroutine.status,
	        wrap = coroutine.wrap,
	        yield = coroutine.yield, -- Fixed typo
	    },
	    debug = debug,
	    math = math,
	    os = {
	    ...
	    string = string,
	    table = table,
	    task = task, -- Included full library
	    utf8 = utf8,
	    vector = vector,
